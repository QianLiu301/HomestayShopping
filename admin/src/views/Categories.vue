<template>
  <div>
    <div class="page-header">
      <span></span>
      <el-button type="primary" @click="openDialog()"><el-icon><Plus /></el-icon> Add Category</el-button>
    </div>
      ...
      <el-table :data="list" v-loading="loading" stripe>
        ...
        <el-table-column label="Actions" width="160" fixed="right">
          <template #default="{ row }">
            ...
            <el-popconfirm title="Delete this category?" @confirm="onDelete(row.id)">
    ...
    <el-dialog v-model="dialogVisible" :title="isEdit ? 'Edit Category' : 'Add Category'" width="500px" destroy-on-close>
      <el-form :model="form" label-position="top">
        ...
        <el-row :gutter="16">
          <el-col :span="8"><el-form-item label="Icon"><el-input v-model="form.icon" placeholder="e.g. 🎁" /></el-form-item></el-col>
...
async function onSave() {
  ...
  saving.value = true
  try {
    if (isEdit.value) { await updateCategory(editId.value, form); ElMessage.success('Updated') }
...
async function onDelete(id) {
  try { await deleteCategory(id); ElMessage.success('Deleted'); loadData() } catch {}
}
